...
from modules.jenkins_job_generator.jenkins_job_generator import JobBuilder, ActionRunner
...
<flow-definition plugin="workflow-job">
    ...
    def setup(self):
        def stub(self):
            pass
        old_init = JobBuilder.__init__
        JobBuilder.__init__ = stub
        self.job_builder = JobBuilder()
        JobBuilder.__init__ = old_init
    ...
    def test_get_action_method(self):
        ...
        assert action.__getattribute__(params['action']) is not None

    # Mock tests

    @patch('jenkins.Jenkins.get_job_config')
    def test_jobs_not_update(self, mock):
        result = dict(
            changed=False,
            original_message='',
            message=''
        ...
        with open('job.yaml', 'w') as f:
            f.write(self.scm_yaml)
            f.write('\n')
            f.write(self.build_job_yaml)
            f.write('\n')
            f.write(self.test_job_yaml)

        mock.return_value = self.build_job_xml

        action = ActionRunner(result=result, **params)

        result = action.__getattribute__(params['action'])(**params)

        assert not result['changed']

    @patch('jenkins.Jenkins.get_job_config')
    @patch('jenkins_jobs.builder.JenkinsManager.update_job')
    def test_jobs_update(self, update_job_mock, get_job_config_mock):
        result = dict(
            changed=False,
            original_message='',
            message=''
    ...
    @patch('jenkins.Jenkins.get_job_config')
    @patch('jenkins_jobs.builder.JenkinsManager.update_job')
    def test_jobs_create(self, update_job_mock, get_job_config_mock):
        result = dict(
            changed=False,
            original_message='',
            message=''
